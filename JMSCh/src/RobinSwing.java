 ...
 *
 */
public class RobinSwing extends JFrame implements ActionListener {
	 ...
	 */
	public RobinSwing() throws HeadlessException {
		...
		try {
			...
			destinationProducer = session.createQueue("SAM");
			producer = session.createProducer(destinationProducer);
		} catch (JMSException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		new RobinReceiver(chat).start();
	}

	/**
	 * @param gc
	 */
	public RobinSwing(GraphicsConfiguration gc) {
		super(gc);
	 ...
	 * @param gc
	 */
	public RobinSwing(String title, GraphicsConfiguration gc) {
		super(title, gc);
		// TODO Auto-generated constructor stub
	}
	 ...
	 * @throws HeadlessException
	 */
	public RobinSwing(String title) throws HeadlessException {
		super(title);
		// TODO Auto-generated constructor stub
	}
	...
	public static void main(String[] args) {
		...
		RobinSwing frame = new RobinSwing();
		...
		if (e.getSource() == send) {
			synchronized (chat) {
				...
				try {
					...
					text.setStringProperty("PRODUCER_NAME", "Robin");
					producer.send(text);
					message.setText("");
...
class RobinReceiver extends Thread {
	JTextArea chat;
	private ConnectionFactory factory = null;
	private Connection connection = null;
	private Session session = null;
	...
	public RobinReceiver(JTextArea chat) {
		this.chat = chat;
		factory = new ActiveMQConnectionFactory(ActiveMQConnection.DEFAULT_BROKER_URL);
	...
	public void initMQ() {
		try {
			...
			destinationConsumer = session.createQueue("ROBIN");
